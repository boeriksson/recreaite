import React from 'react';
import { motion } from 'framer-motion';
import { base44 } from '@/api/base44Client';
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { X, Mail, Sparkles } from 'lucide-react';

export default function LoginModal({ onClose, darkMode }) {
  const [email, setEmail] = React.useState('');
  const [loading, setLoading] = React.useState(false);

  const handleEmailLogin = async () => {
    if (!email) return;
    setLoading(true);
    try {
      // Redirect to login with email
      base44.auth.redirectToLogin(window.location.href);
    } catch (error) {
      console.error('Login failed:', error);
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        className={`${darkMode ? 'bg-[#1a1a1a] border-white/10' : 'bg-white border-black/10'} rounded-3xl max-w-md w-full border p-8`}
      >
        <button
          onClick={onClose}
          className={`absolute top-4 right-4 p-2 rounded-full ${darkMode ? 'hover:bg-white/10' : 'hover:bg-black/5'} transition-colors`}
        >
          <X className="h-5 w-5" />
        </button>

        <div className="text-center mb-8">
          <div className="h-16 w-16 rounded-full bg-[#392599] flex items-center justify-center mx-auto mb-4">
            <Sparkles className="h-8 w-8 text-white" />
          </div>
          <h2 className="text-2xl font-semibold mb-2">Välkommen till HeyLook</h2>
          <p className={darkMode ? 'text-white/60' : 'text-black/60'}>
            Logga in för att börja generera bilder
          </p>
        </div>

        <div className="space-y-4">
          <div>
            <Input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder="din@email.com"
              className={`${darkMode ? 'bg-white/5 border-white/20 text-white' : 'bg-[#f5f5f7] border-black/10'}`}
              onKeyDown={(e) => e.key === 'Enter' && handleEmailLogin()}
            />
          </div>

          <Button
            onClick={handleEmailLogin}
            disabled={loading || !email}
            className="w-full bg-[#392599] hover:bg-[#4a2fb3] text-white rounded-full"
          >
            <Mail className="h-4 w-4 mr-2" />
            Fortsätt med Email
          </Button>

          <div className={`relative my-6 ${darkMode ? 'text-white/40' : 'text-black/40'}`}>
            <div className={`absolute inset-0 flex items-center`}>
              <div className={`w-full border-t ${darkMode ? 'border-white/10' : 'border-black/10'}`}></div>
            </div>
            <div className="relative flex justify-center text-xs">
              <span className={`px-2 ${darkMode ? 'bg-[#1a1a1a]' : 'bg-white'}`}>eller</span>
            </div>
          </div>

          <Button
            onClick={() => base44.auth.redirectToLogin(window.location.href)}
            variant="outline"
            className={`w-full rounded-full ${darkMode ? 'border-white/20 text-white hover:bg-white/10' : 'border-black/10 hover:bg-black/5'}`}
          >
            <svg className="h-5 w-5 mr-2" viewBox="0 0 24 24">
              <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Google
          </Button>

          <Button
            onClick={() => base44.auth.redirectToLogin(window.location.href)}
            variant="outline"
            className={`w-full rounded-full ${darkMode ? 'border-white/20 text-white hover:bg-white/10' : 'border-black/10 hover:bg-black/5'}`}
          >
            <svg className="h-5 w-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09l.01-.01zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/>
            </svg>
            Apple
          </Button>
        </div>

        <p className={`text-xs text-center mt-6 ${darkMode ? 'text-white/40' : 'text-black/40'}`}>
          Genom att fortsätta godkänner du våra användarvillkor
        </p>
      </motion.div>
    </div>
  );
}